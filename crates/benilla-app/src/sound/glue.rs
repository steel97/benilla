...
/// The held glue-music stream (non-`Sync` handle — non-Send state, like [`super::zone`]'s).
#[derive(Default)]
struct GlueMusic(Option<mixer::StreamingSoundHandle<kira::sound::FromFileError>>);
...
/// Entering the world: fade the theme out on the backend and drop the handle.
fn stop_glue_music(mut music: NonSendMut<GlueMusic>) {
    if let Some(mut h) = music.0.take() {
