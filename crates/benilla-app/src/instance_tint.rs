...
pub(crate) struct InstanceTints {
    slots: Arc<Vec<u32>>,
    generation: u64,
}
...
impl InstanceTints {
    ...
    pub(crate) fn set(&mut self, slot: u16, word: u32) {
        let i = slot as usize;
    ...
    /// This slot's current word (`IDENTITY` when untinted, or for an out-of-range slot). Test-only:
    /// the render path reads the uploaded region, never this.
    #[cfg(test)]
    pub(crate) fn get(&self, slot: u16) -> u32 {
