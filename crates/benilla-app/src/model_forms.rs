...
/// lane rigs.
#[derive(Clone, Copy)]
pub(crate) struct FormSlices<'a> {
    pub(crate) stat: &'a [(Handle<Mesh>, Option<Aabb>)],
    pub(crate) skin: Option<&'a [Handle<Mesh>]>,
}
...
pub(crate) fn furnish_model_forms(
    ...
    let live = player
        .as_deref()
        .is_some_and(|p| p.active && !p.settling && !p.world_stale);
    let cap = if live { MESH_BUDGET } else { usize::MAX };
