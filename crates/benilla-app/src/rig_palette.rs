...
//! (planted by `entities::spawn_joints`), and [`compute_rig_palettes`] runs one linear
...
impl RigPalettes {
    ...
    pub(crate) fn mark_mirrored(&mut self, slot: u16) {
    ...
    pub(crate) fn world_palette(&self, slot: u16, bones: usize) -> Option<Vec<Mat4>> {
        let s = slot as usize;
    ...
    pub(crate) fn computed_rigs(&self) -> usize {
        (0..self.slot_high)
    ...
    pub(crate) fn occupancy(&self) -> (usize, u32, usize, u32) {
        (
...
/// field instead.
#[derive(Component)]
pub(crate) struct RigPart(pub(crate) Entity);
...
pub(crate) struct RigPaletteMirrors(
    pub(crate) std::collections::HashMap<&'static str, bevy::render::render_resource::Buffer>,
);
