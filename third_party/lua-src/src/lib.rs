...
impl AddFilesByExt for cc::Build {
    fn add_files_by_ext(&mut self, dir: &Path, ext: &str) -> Result<&mut Self, Box<dyn Error>> {
        ...
        // BENILLA ADDITION. Upstream emits no `rerun-if-changed` for the Lua sources, which is
        // harmless when they are an immutable registry copy and a genuine trap once they are a
        // patched fork: editing `lvm.c` then rebuilds NOTHING, the old object file is relinked, and
        // the change appears not to work. That cost real time on the very first patch here — six
        // tests failed against a stale VM and the obvious conclusion was that the patch was wrong.
        // `Build::build` runs inside mlua-sys's build script, so anything printed here is that
        // script's output and cargo honours it.
        //
        // **Emitted for EVERY file in the directory, not just the ones being compiled.** The first
        // version of this tracked only the `ext` matches — i.e. `.c` — and the identical trap came
        // straight back one file-type over: flipping `LUA_COMPAT_LSTR` in `luaconf.h` rebuilt
        // nothing and the new test failed against a stale object file, exactly as before. A header
        // is an input to every translation unit here; so is anything else that lands in this
        // directory, which is why this no longer filters.
        for entry in fs::read_dir(dir)
            .context(|| format!("Cannot read '{}'", dir.display()))?
            .filter_map(|e| e.ok())
        {
